...
def loadRSS(): 
    ...
    # saving the xml file 
    with open('sportsfeed.xml', 'wb') as f: 
...
def main(): 
    ...
    # parse xml file 
    newsitems = parseXML('sportsfeed.xml') 
  
    # store news items in a csv file 
    savetoCSV(newsitems, 'sportsnews.csv') 
...
df_survey_data = pd.read_csv("sportsnews.csv")
...
df.to_csv("sportsheadlines.csv", mode='w', columns=COLS, index=False, encoding="utf-8")

# link file

with open("sportsnews.csv", "r") as csv_file:
   csv_reader = csv.DictReader(csv_file)

   with open("sportslink.csv","w") as new_file:
    fieldnames=['link']
...
art = []
with open('sportslink.csv', 'r+') as csvfile:
...
with open('sportssummary.csv', 'w', encoding='utf-8', newline='') as file:
...
df = pd.read_csv('sportsheadlines.csv')
titles = list(df.iloc[:, 0].values)
sentiment = list(df.iloc[:, 2].values)

df_link = pd.read_csv('sportslink.csv')
links = list(df_link.iloc[:, 0].values)

df_sum = pd.read_csv('sportssummary.csv')
