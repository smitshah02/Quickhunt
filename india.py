...
def loadRSS(): 
    ...
    # saving the xml file 
    with open('indianewsfeed.xml', 'wb') as f: 
...
def main(): 
    ...
    # parse xml file 
    newsitems = parseXML('indianewsfeed.xml') 
  
    # store news items in a csv file 
    savetoCSV(newsitems, 'indianews.csv') 
...
df_survey_data = pd.read_csv("indianews.csv")
...
df.to_csv("indiaheadlines.csv", mode='w', columns=COLS, index=False, encoding="utf-8")

# link file

with open("indianews.csv", "r") as csv_file:
   csv_reader = csv.DictReader(csv_file)

   with open("indialink.csv","w") as new_file:
    fieldnames=['link']
...
art = []
with open('indialink.csv', 'r+') as csvfile:
...
with open('indiasummary.csv', 'w', encoding='utf-8', newline='') as file:
...
df = pd.read_csv('indiaheadlines.csv')
titles = list(df.iloc[:, 0].values)
sentiment = list(df.iloc[:, 2].values)

df_link = pd.read_csv('indialink.csv')
links = list(df_link.iloc[:, 0].values)

df_sum = pd.read_csv('indiasummary.csv')
