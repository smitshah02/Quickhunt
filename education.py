...
df_survey_data = pd.read_csv("edunews.csv")
...
df.to_csv("eduheadlines.csv", mode='w', columns=COLS, index=False, encoding="utf-8")

# link file

with open("edunews.csv", "r") as csv_file:
   csv_reader = csv.DictReader(csv_file)

   with open("edulink.csv","w") as new_file:
    fieldnames=['link']
...
art = []
with open('edulink.csv', 'r+') as csvfile:
...
with open('edusummary.csv', 'w', encoding='utf-8', newline='') as file:
...
df = pd.read_csv('eduheadlines.csv')
titles = list(df.iloc[:, 0].values)
sentiment = list(df.iloc[:, 2].values)

df_link = pd.read_csv('edulink.csv')
links = list(df_link.iloc[:, 0].values)

df_sum = pd.read_csv('edusummary.csv')
