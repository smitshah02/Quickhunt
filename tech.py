...
def loadRSS(): 
    ...
    # saving the xml file 
    with open('technewsfeed.xml', 'wb') as f: 
...
def main(): 
    ...
    # parse xml file 
    newsitems = parseXML('technewsfeed.xml') 
  
    # store news items in a csv file 
    savetoCSV(newsitems, 'technews.csv') 
...
df_survey_data = pd.read_csv("technews.csv")
...
df.to_csv("techheadlines.csv", mode='w', columns=COLS, index=False, encoding="utf-8")

# link file

with open("technews.csv", "r") as csv_file:
   csv_reader = csv.DictReader(csv_file)

   with open("techlink.csv","w") as new_file:
    fieldnames=['link']
...
art = []
with open('techlink.csv', 'r+') as csvfile:
...
with open('techsummary.csv', 'w', encoding='utf-8', newline='') as file:
...
df = pd.read_csv('techheadlines.csv')
titles = list(df.iloc[:, 0].values)
sentiment = list(df.iloc[:, 2].values)

df_link = pd.read_csv('techlink.csv')
links = list(df_link.iloc[:, 0].values)

df_sum = pd.read_csv('techsummary.csv')
